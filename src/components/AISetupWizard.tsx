...
export function AISetupWizard({ catalog, status, onStatus, onReady, onClose }: Props) {
  ...
  const connect = async () => {
    if (!selected || !consent || !apiKey.trim()) return;
    setBusy(true);
    setError('');
    try {
      const saved = await desktop.aiConfig.saveDraft({ presetId: selected.id, apiKey, fields, consentAccepted: consent });
      ...
      <section className="ai-setup-dialog" role="dialog" aria-modal="true" aria-labelledby="ai-setup-title">
        ...
        {step === 1 && selected ? (
          <>
            ...
            <label className="ai-setup-field">访问密钥<input type="password" autoComplete="off" value={apiKey} onChange={(event) => setApiKey(event.target.value)} placeholder="粘贴后由 Windows 加密保存" /></label>
            <div className="ai-data-boundary">
              <ShieldCheck /><div><strong>发送范围</strong><p>初次建库发送内置古籍片段；解卦时发送当前问题、排盘与命中的证据；不会发送全部历史。费用由服务商收取，问爻不会代扣。</p></div>
            </div>
            <label className="ai-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /><span>我已了解上述数据发送范围和第三方费用</span></label>
            {error ? <p className="ai-setup-error" role="alert">{error}</p> : null}
            <div className="ai-setup-actions"><button type="button" onClick={() => setStep(0)}><ArrowLeft size={15} />返回</button><button className="primary" type="button" disabled={busy || !consent || !apiKey.trim() || (selected.requiredFields || []).some((field) => !fields[field.id]?.trim())} onClick={() => void connect()}><KeyRound size={15} />{busy ? '正在检测…' : '保存并检测三项能力'}</button></div>
          </>
        ) : null}

        {step === 2 ? (
          <>
            <header><p>准备知识库</p><h2 id="ai-setup-title">检测与构建正在进行</h2><span>窗口可以关闭，任务会继续在后台运行；退出应用后也能从已完成批次继续。</span></header>
        ...
        {step === 3 ? (
          <div className="ai-setup-complete"><Check /><p>AI 服务已就绪</p><h2 id="ai-setup-title">三项能力和向量索引均已通过</h2><span>现在可以使用带古籍向量召回与重排的 AI 解读。</span><button type="button" onClick={onReady}>开始解读</button></div>
        ) : null}
      </section>
    </div>
  );
