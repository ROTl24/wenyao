...
export function RitualScreen({ session, onConfirm }: Props) {
  ...
  useEffect(() => {
    ...
    const settleTimer = window.setTimeout(() => setSettled(true), reduced ? 180 : session.tosses.length === 0 ? 2550 : 1750);
  ...
  return (
    <main className="ritual-screen">
      ...
      <button className="ritual-stage" type="button" onClick={() => { setOpened(true); setSettled(true); }} aria-label="起卦动画，点击可直接查看结果">
        <div className={opened ? 'ink-hands ink-hands--open' : 'ink-hands ink-hands--closed'} aria-hidden="true" />
        <Suspense fallback={<div className="coin-scene-loading">正在唤醒铜钱…</div>}><CoinScene faces={current.faces} /></Suspense>
