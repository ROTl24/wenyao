...
export function SettingsPanel({ onClose }: Props) {
  ...
  const [corpus, setCorpus] = useState({ count: 0, originalCount: 0, summaryCount: 0, ready: false });
  ...
  return (
    <div className="overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <aside className="side-panel settings-panel" aria-modal="true" role="dialog">
        ...
        <section className="settings-section"><div className="settings-heading"><Database /><div><strong>本地古籍证据包</strong><span>整本古籍不会上传给云端 AI</span></div></div><div className="corpus-stats"><span><b>{corpus.count}</b>条证据</span><span><b>{corpus.originalCount}</b>条原文</span><span><b>{corpus.summaryCount}</b>条摘要</span></div>{corpus.originalCount === 0 && <p className="corpus-warning">当前为演示摘要。把你的古籍交给我后，会替换为带书名、版本、页码与原页高亮的正式证据包。</p>}</section>
