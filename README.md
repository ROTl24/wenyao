...
一款本地优先的水墨六爻应用，提供 Windows 桌面版与可安装的手机 PWA。用户输入所占之事后，可选择在线 3D 投掷、线下实体铜钱录入、安全随机一键成卦，或按北京时间使用梅花年月日时法成卦；程序统一按文王纳甲规则排盘。桌面版还可结合本地古籍证据与用户自行配置的云端模型生成解读并支持继续追问。
...
AI 解读、追问、用户古籍导入与向量索引仍属于 Windows 桌面版能力。PWA 不保存 API Key，也不会为了手机版把“解读、向量召回、专用重排”质量契约降级成不完整方案。
