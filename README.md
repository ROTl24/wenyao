...
- 服务商预设与可选检索增强，主模型即可完成配置
- 阿里云百炼 / SiliconFlow / DeepSeek
- 任意 OpenAI 兼容接口
- 同卦追问，每条独立反馈
- 长解读边生成边阅读，可停止接收并保留未完成草稿
- 结论速览、章节导航、事项示例与术语解释
...
| 版本 | 更新内容 |
|:---:|---------|
| **v0.5.7** | 长解读边生成边阅读、停止与草稿、应用内后台任务；结论与章节导航、占簿备份恢复和事后复盘；事项模板、术语解释及更直接的 AI 配置 |
| **v0.5.6** | 修复 Windows 状态文件短暂占用导致的进度保存中断；网页 AI 收到完整流式结果后不再因底层流清理失败误报网络错误 |
| **v0.5.5** | 远程向量建库按完整批次保存可恢复断点，失败后要求重新测试或显式降级，避免重复发送可能计费的请求；排盘复制明确区分成卦变化与变爻回头作用方向 |
| **v0.5.4** | 同步桌面端与 PWA 的古籍分类、自定义 `/v1` 地址补全、通用 Chat 响应解析及模型目录域名确认修复；Windows 旧版本通过应用内更新提示升级 |
