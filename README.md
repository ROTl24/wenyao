...
问爻会把占卜历史、模型设置、DPAPI 加密后的 API Key、向量索引和 Electron 运行数据统一保存在所选安装目录的 `data\` 文件夹，不再使用 `%APPDATA%\liuyao-divination` 作为正式数据目录。升级和卸载只替换或移除程序文件，`data\` 会原地保留；如需彻底清除个人数据，请在卸载后手动删除该文件夹。旧版本位于 `%APPDATA%` 的数据会在新版首次启动时逐文件校验迁移，确认复制完整后才删除旧目录。

安装目录必须允许当前 Windows 用户写入。建议选择安装器默认的当前用户目录，或用户在其他磁盘上拥有完全控制权限的文件夹；不要选择只允许管理员写入的受保护目录，否则应用会明确停止启动，避免出现历史或密钥无法保存的假成功。
