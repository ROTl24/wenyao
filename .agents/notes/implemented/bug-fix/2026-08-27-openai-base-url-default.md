...
地址用法分为自动补全和完整接口。完整接口模式保留根路径、非标准能力路径、尾斜线及非敏感查询参数，不追加能力后缀；界面预览最终请求地址，测试与正式调用共用同一端点。已保存连接按完整能力地址恢复，协议仍为对应能力的既有兼容协议。
...
非标准网关可选择完整接口模式，模型目录不可用时直接手填模型即可测试。地址自由配置不改变 HTTPS、网页公网与 CORS 边界，也不推断另一种生成协议。
