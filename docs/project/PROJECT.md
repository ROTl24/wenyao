...
| `0.5.7` 跨渠道发布 | `verified` | GitHub 源码、稳定 Release 五项资产与网站资源来自同一发布实现 | `v0.5.7` / `b22c0c8`、GitHub Actions `33990404576`、远端资产下载及网站摘要校验 |
...
- `0.5.7` 已发布到 GitHub 稳定 Release、Windows/macOS 安装包及 Cloudflare Pages；源码标签固定于 `b22c0c8`。未执行真实付费模型调用或本机静默安装。
...
- `v0.5.7` 正式 Release 提供 Windows NSIS、blockmap、`latest.yml`、macOS 通用 DMG 与 SHA-256 清单；Cloudflare Pages 的主 JS、CSS、AI Worker 和 manifest 与该版本本地构建摘要一致，Service Worker 预缓存同一运行资源。
...
- 使用 0.5.7 并收集实际体验；真实模型质量基线仍需获准样本，不包含自动付费调用。
...
| `G-019` | `verified` | 2026-09-06 | `v0.5.7` 源码、稳定 Release、五项远端资产、双架构 macOS 和网站运行资源核验完成 |
