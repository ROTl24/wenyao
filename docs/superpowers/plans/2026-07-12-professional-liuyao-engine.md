...
- `yehe_core_v1` 对日冲采用保守分类：先始终输出结构性 `clashes`；只有月令同支、同五行或月令生爻且不存在月破时，才输出条件性 `is-dark-moving`；只有月破或月令克爻且没有任何已记录生扶时，才输出条件性 `is-day-break`；其余只保留日冲原始事实，不强判暗动或日破。
- 受限神煞的取法必须登记为：天乙贵人、禄神以日干起；驿马以日支三合局起；天喜以年支起。四项都只能输出 `secondary + conditional`，不得进入旺衰评分或单独定吉凶。
