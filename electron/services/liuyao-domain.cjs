...
const CATEGORY_USE_GOD_RULES = {
  ...
  travel: {
    subjects: ['出行', '行程', '旅行', '行人'],
    selectionMode: 'question-specific',
    secondaryRelations: [],
    rule: '出行事项必须先根据所问是自身出行还是他人行踪确定用神，再看世应、动爻与日月。',
  },
  other: {
    subjects: ['此事', '此问', '计划', '方案', '目标', '行动', '结果'],
    selectionMode: 'question-specific',
    secondaryRelations: [],
    rule: '先根据具体事项在父母、官鬼、妻财、子孙、兄弟或世应中确定主用神，再进入旺衰生克分析。',
  },
};
const QUESTION_SUBJECTS = [...new Set(Object.values(CATEGORY_USE_GOD_RULES).flatMap((rule) => rule.subjects))];
